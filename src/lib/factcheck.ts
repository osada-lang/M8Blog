...
export async function runFactCheck(
  ...
  if (promptType === 'medical') {
    ...
    // 免責事項の有無チェック
    if (!content.includes('免責') && !content.includes('保証するものではありません') && !content.includes('医師の診察')) {
      issues.push({
        ...
        suggestion: '「※本記事は情報提供を目的としており、特定の効果を保証するものではありません。」を追記してください。',
      });
    }
  }
  ...
  let summary = '';
  if (issues.length === 0) {
    summary = 'ファクトチェック合格: ナレッジとの不整合や薬機法等のリスクは見つかりませんでした。';
  } else {
    const highCount = issues.filter(i => i.severity === 'high').length;
    if (highCount > 0) {
      summary = `重大な注意点が${highCount}件検出されました。公開前に該当箇所の修正を強く推奨します。`;
    } else {
      summary = `${issues.length}件の確認推奨事項があります。プレースホルダーや表現をご確認ください。`;
    }
  }
...
/**
 * Claudeを使った高度なファクトチェック（ハルシネーション検証）
 */
async function runLlmFactCheck(
  ...
  const prompt = `以下の【元資料（ナレッジ）】と【生成されたブログ記事】を照合し、ハルシネーション（元資料に根拠がない断定的な創作）や、医療系リスク（誇大広告、薬機法違反リスク）をチェックしてください。
...
【モード】: ${promptType === 'medical' ? '医療系（薬機法・医療広告ガイドライン適用）' : '普通（一般店舗・企業）'}
